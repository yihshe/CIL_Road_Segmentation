...
def build_train_val(train_path, val_path, val_size=0.2, seed=1):
    ...
    for im in val_images:
        ...
        os.rename(os.path.join(train_path, 'groundtruth2', im), os.path.join(val_path, 'groundtruth', im))
...
def trainvalGenerator(batch_size, aug_dict, 
    ...
    return (pair for pair in zip(image_generator_train, mask_generator_train)), (pair for pair in zip(image_generator_val, mask_generator_val))


def testGenerator(test_img_list, num_image = 94):
...
def save_result(save_path, npyfile, indices):
    ...
    for i, item in enumerate(npyfile):
        ...
        io.imsave(os.path.join(save_path, '%d.png'%(indices[i])), rescaled)
        # io.imsave(os.path.join(save_path, '%d.png'%(indices[i])), img)
