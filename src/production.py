...
from models.deepsmileres import UNet
...
DATA_SIZE = 100 # 100
TRAINING_SIZE, TESTING_SIZE = 100, 94
PERM = True
BATCH_SIZE, NUM_EPOCHS = 16, 20
...
def run(cuda, gpu_idx, train):
    ...
    # Generate submission
    generate_predictions(TESTING_SIZE, TEST_IMG_SIZE, TEST_PATCH_SIZE, img_patches_submit, TEST_LABEL_PATH, testset.nblist)
    generate_submission_csv(TESTING_SIZE, SUBMISSION_PATH, TEST_LABEL_PATH, testset.nblist)
...
def train_model(model, cuda, gpu_idx, aug_config):
    ...
    indices = np.arange(1, TRAINING_SIZE + 1)
    trainset = PatchedAerialDataset(TRAIN_IMAGE_PATH, TRAIN_LABEL_PATH, indices, PATCH_SIZE, OVERLAP, OVERLAP_AMOUNT, aug_config)
    ...
    training(NUM_EPOCHS, model, criterion, optimizer, lr_scheduler, datasets, dataloaders, PATCH_SIZE, validate=False, cuda=cuda, gpu_idx=gpu_idx)
